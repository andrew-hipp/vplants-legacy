...
<xsl:value-of select="/SEARCH/PARAMETERS/db"/>
</xsl:variable>
...
<xsl:value-of select="/SEARCH/PARAMETERS/ssp"/>
</xsl:variable>

<xsl:variable name="variety">
<xsl:value-of select="/SEARCH/PARAMETERS/variety"/>
</xsl:variable>

<xsl:variable name="forma">
<xsl:value-of select="/SEARCH/PARAMETERS/forma"/>
</xsl:variable>
...
<xsl:choose>
  ...
  <xsl:when test='$searchType = "1"'>
	...
	method="post" 
	action="/xsql/common/searchtaxon.xsql">
      ...
      <input type="hidden" name="ssp" value="{$ssp}"/>
      ...
      <a href="javascript:document.frm{GROUP_ID}.submit();">
          <xsl:apply-templates/>
      </a>
    </form>
  </xsl:when>
