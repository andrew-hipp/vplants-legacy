...
<xsl:if test="SUBSTRATUM">
 <tr>
  <td class="text">
   <strong>Substratum:</strong> <xsl:value-of select="SUBSTRATUM"/><br />
  </td>
 </tr>
</xsl:if>
<tr>
...
<xsl:template match="DETS_ROW[last()]">
  <xsl:if test="F_NAME">
   <xsl:value-of select="F_NAME"/>.
  </xsl:if>
  <xsl:if test="M_NAME">
